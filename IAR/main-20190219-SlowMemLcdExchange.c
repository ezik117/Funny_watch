...
{
  uint8_t c;
  //закончим все передачи и опустошим буфер приема
  while ((SPI1->SR & SPI_SR_TXE) != SPI_SR_TXE);
  while ((SPI1->SR & SPI_SR_RXNE) == SPI_SR_RXNE) c = SPI1->DR;
  ...
  for (uint8_t hiByte=0; hiByte<(uint8_t)(BmpHeader16.length>>8); hiByte++)
  {
  ...
  for (uint8_t loByte=0; loByte<((uint8_t)BmpHeader16.length); loByte++)
  {
    ramImg.buf[loByte] = SPI_Exchange(0xFF);
  }

  MEM_PAUSE;
  LCD_CS(CS_ENABLE);
  for (uint8_t loByte=0; loByte<((uint8_t)BmpHeader16.width); loByte++)
  {
    SPI_Exchange(ramImg.buf[loByte]);
  }  
  LCD_CS(CS_DISABLE);
  MEM_RESUME;     
