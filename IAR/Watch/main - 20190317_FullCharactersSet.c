...
int main()
{
  ...
  v.mCalendarAlarmTime = 0x00000000;
  for (uint8_t i=0; i<7; i++) v.DoW_Checked[i] = (uint8_t)0;
  ...
  while (true)
  {
    ...
    if (v.tObjectID != 0)
    {
        ...
        {      
          ...
          switch (v.tObjectID)
          {
            ...
            case 0x83:
              ...
              if (flags.systemState == 9)
              {
                ...
                for (uint8_t i=0; i<4; i++)
                {
                  ...
                  LCD_ShowText(55, 8 + i*47, (uint8_t*)&TShortDAYS[j], 2, LCD_BLACK, (v.digitsOffset == 0 ? LCD_CYAN : LCD_YELLOW));
                    j += 3;
                  LCD_FillRectangle(161, 319, i*47, i*47+44, (v.digitsOffset == 0 ? LCD_CYAN : LCD_YELLOW));
                    LCD_ShowText(216, 8 + i*47, (uint8_t*)&TShortDAYS[j], 2, LCD_BLACK, (v.digitsOffset == 0 ? LCD_CYAN : LCD_YELLOW));
                    j += 3;
                }
                //отобразим чекбоксы
              ...
              {              
                LCD_FillRectangle(0, 319, 0, 239, BackGroundColor);
                LCD_ShowText(112, 60, "2019\0", 2, LCD_CYAN, BackGroundColor);
                LCD_ShowText(64, 60+40, "ЗАБАВНЫЕ\0", 2, LCD_YELLOW, BackGroundColor);
                LCD_ShowText(112, 60+40+40, "ЧАСЫ\0", 2, LCD_YELLOW, BackGroundColor);
                //установим флаги
                flags.systemState = 7;
                flags.menuIsShowed = false;
              }
      ...
      if ((flags.systemState == 10) && (v.tObjectID != 0))
      {
        ...
        if (v.DoW_Checked[v.tObjectID - 1])
          v.DoW_Checked[v.tObjectID - 1] = (uint8_t)0;
        else
          v.DoW_Checked[v.tObjectID - 1] = v.tObjectID;
        //
        Alarm_ShowDowCheckboxes();
        //
        v.tObjectID = 0;
...
void LCD_ShowText(uint16_t x, uint8_t y, uint8_t* address, uint8_t font, uint16_t fcolor, uint16_t bcolor)
{
  ...
  while (*address != 0)
  {
    ...
    else
    {
      ...
      {
//        if (s == 0x20) s = 43; //пробел
//        if (s == 0x2D) s = 42; //дефис
//        if (s == 0x2E) s = 44; //точка
      }
    }
...
void Alarm_ShowDowCheckboxes()
{
  ...
  for (uint8_t i=1; i<8; i++)
  {
    ...
    //отобразим
    if (v.DoW_Checked[i-1] == 0)
      LCD_ShowText(x, y, " \0", 2, 0x0680, (v.digitsOffset == 0 ? LCD_CYAN : LCD_YELLOW));
    else
      LCD_ShowImage2FromRom(x, y, 42, 2, 0x0680, (v.digitsOffset == 0 ? LCD_CYAN : LCD_YELLOW));
  }
}
