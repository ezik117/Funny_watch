...
struct { //структура управления 1wire 
  volatile uint8_t Start; //=1 - запустить программу, программа заканчивается когда бит 0 установлен в 0  
  volatile uint8_t RetVal; //считанное значение из линии, если программа содержит инструкцию чтения
  volatile uint8_t* Program; //указатель на программу
...
int main()
{
  ...
  RCC->APB1ENR |= RCC_APB1ENR_TIM3EN | RCC_APB1ENR_TIM14EN; //включить тактирование TIM3, TIM14
  //включить RTC
...
//  b = (GPIOA->IDR & GPIO_IDR_3) >> 3;
  ...
  TIM3->DIER |= TIM_DIER_UIE;
   ...
   //--- настройка таймера TIM14 (работа с 1-wire протоколом)
  ow.Start = 0;
  TIM14->PSC = 47; //1us
  TIM14->ARR = 1; //1ms
...
{
  if ((ow.Start & 0x01) == 1) //программа запущена
  {
    if ((ow.Start & 0x02) == 0) //загрузка программы
    {
      ow._mode = *ow.Program;
      ow.Program++;
      ...
      ow.Start |= 0x02; //загрузка завершена
    }
    else
    {
      switch (ow._mode)
      {
        ...
        case 1: //прижать линию к GND
          GPIOA->BSRR = GPIO_BSRR_BR_3;
        break;
        
        case 2: //отпустить линию к VCC
          GPIOA->BSRR = GPIO_BSRR_BS_3;
        break;      
        
        case 3: //отпустить линию к VCC и считать данные
          GPIOA->BSRR = GPIO_BSRR_BS_3;
      ...
      if (ow._cycles == 0)
      {
        ow.Start &= ~0x02; //сбросим бит загрузки программы
      }
    }
  }
